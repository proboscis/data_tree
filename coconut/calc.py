...
tokens = ('LPAREN', 'RPAREN', 'LSB', 'RSB', 'COMMA', 'NAME', 'STR',)
...
class ATuple(_coconut.collections.namedtuple("ATuple", "items"), _coconut.object):
    __slots__ = ()
    ...
    def __new__(_cls, *items):
        return _coconut.tuple.__new__(_cls, items)
    @_coconut.classmethod
    def _make(cls, iterable, new=_coconut.tuple.__new__, len=None):
        return new(cls, iterable)
    def _asdict(self):
        return _coconut.OrderedDict([("items", self[:])])
    def __repr__(self):
        return "ATuple(*items=%r)" % (self[:],)
    def _replace(_self, **kwds):
        result = self._make(kwds.pop("items", _self))
        if kwds:
...
class AList(_coconut.collections.namedtuple("AList", "items"), _coconut.object):
    __slots__ = ()
    __ne__ = _coconut.object.__ne__
    def __eq__(self, other):
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)
...
_coconut_match_to = res
_coconut_case_check_0 = False
if (_coconut.isinstance(_coconut_match_to, _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to) == 2) and (_coconut_match_to[0] == "a") and (_coconut_match_to[1] == "b"):
